...
from src.utility.models import DataClip, DataMark
...
class InfoDramaScraper(BaseScraper):
    ...
    def _get_creator(self) -> List[str] | None:
        ...
        return [creator.text for creator in title_elem.find_next_sibling("ul").find_all("div")] if title_elem else None
    
    def _get_scriptwriter(self) -> List[str] | None:
        title_elem = self.soup.find("h3", class_="p-content-detail__people-list-term", string="脚本")

        return [scriptwriter.string for scriptwriter in title_elem.find_next_sibling("ul").find_all("div")] if title_elem else None

    def _get_director(self) -> List[str] | None:
        title_elem = self.soup.find("h3", class_="p-content-detail__people-list-term", string="監督")

        return [director.text for director in title_elem.find_next_sibling("ul").find_all("div")] if title_elem else None

    def _get_artist(self) -> List[str] | None:
        title_elem = self.soup.find("h3", class_="p-content-detail__people-list-term", string="主題歌／挿入歌")

        return [artist.text for artist in title_elem.find_next_sibling("ul").find_all("div")] if title_elem else None
    
    def _get_cast(self) -> List[str] | None:
        title_elem = self.soup.select_one("div.p-people-list__casts")

        return [cast.text for cast in title_elem.select("div.c2-button-tertiary-s-multi-text__text")] if title_elem else None
