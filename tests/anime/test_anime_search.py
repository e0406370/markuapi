...
def test_search_input_not_valid_integer(client_nc, query) -> None:
    resp = client_nc.get(f"/search/animes{query}")
    resp_data = resp.json()
...
def test_search_without_results_page_1(client_nc, caplog) -> None:
    query = '".*&^'
    resp = client_nc.get(f"/search/animes?q={query}")
    resp_data = resp.json()
...
def test_search_without_results_page_2(client_nc, caplog) -> None:
    ...
    resp = client_nc.get(f"/search/animes?q={query}&page=2")
    resp_data = resp.json()
...
def test_search_with_results_single(client_nc, test_data, caplog) -> None:
    query = "俺物語!!"
    resp = client_nc.get(f"/search/animes?q={query}&limit=1")
    resp_data = resp.json()
...
def test_search_with_results_multiple(client_nc, test_data, caplog) -> None:
    query = "のだめカンタービレ"
    resp = client_nc.get(f"/search/animes?q={query}&page=1")
    resp_data = resp.json()
